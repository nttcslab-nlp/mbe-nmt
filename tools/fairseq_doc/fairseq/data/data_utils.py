...
def batch_by_size_doc(
    indices, num_tokens_fn, doc_fn, max_tokens=None, max_sentences=None,
):
    """
    Yield mini-batches of indices bucketed by size. Batches may contain
    sequences of different lengths.
    ...
    Args:
        ...
        max_sentences (int, optional): max number of sentences in each
            batch (default: None).
    """
    logger.info('Creating batches with document boundaries')
    try:
        from fairseq.data.data_utils_fast import batch_by_size_doc_fast
    except ImportError:
        raise ImportError(
            'Please build Cython components with: `pip install --editable .` '
            'or `python setup.py build_ext --inplace`'
    ...
    return batch_by_size_doc_fast(indices, num_tokens_fn, doc_fn, max_tokens, max_sentences)
