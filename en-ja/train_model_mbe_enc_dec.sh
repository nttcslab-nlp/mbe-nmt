...
FAIRSEQ=$PROJECT_DIR/fairseq_doc

SEED=$1

EXP_NAME=big_enc_dec_seed$SEED
...
python $FAIRSEQ/train.py $DATA_DIR \
    --arch transformer_document_wmt_en_de_big \
    --task translation_document \
    --document-tags-pref $DOC_PREFIX \
    --add-shuffled-minibatch \
    --classifier-weights \
    --criterion label_smoothed_cross_entropy_with_classifier_loss \
    --classifier-lambda 1.0 \
    --shuffle-classifier \
    --doc-embed-creation doc-encoder \
    --doc-embed-usage-encoder tag \
    --doc-embed-usage-decoder tag \
    --doc-encoder-layer-norm \
    --doc-decoder-layer-norm \
    --count-padding-tokens \
    --doc-encoder-layers 1 \
    --use-word-embed-for-mt-encoder \
    --optimizer adam \
    --adam-betas '(0.9, 0.98)' \
    --clip-norm 1.0 \
